defmodule VoxPublica.Users do
  ...
  def create_remote(attrs), do: Repo.put(remote_changeset(attrs))
  ...
  def remote_changeset(user \\ %User{}, attrs) do
    User.changeset(user, attrs)
    |> Changesets.cast_assoc(:character, attrs)
    |> Changesets.cast_assoc(:profile, attrs)
  end
  ...
  def by_username(username) do
    with {:error, _} <- Repo.single(by_username_query(username)),
         {:error, _} <- Repo.single(by_username_remote_query(username)) do
      {:error, :not_found}
    end
  end
  ...
  def by_username_remote_query(username) do
    from u in User,
      join: p in assoc(u, :profile),
      join: c in assoc(u, :character),
      where: c.username == ^username,
